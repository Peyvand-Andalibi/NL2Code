...
class Learner(object):
    def __init__(self, model, train_data, val_data=None, test_data=None):
        self.model = model
    ...
    def train(self):
        ...
        for epoch in range(nb_epoch):
            ...
            # for batch_index, (examples, batch_ids) in enumerate(train_data_iter):
                ...
                if cum_updates % config.valid_per_batch == 0:
                    ...
                    if config.data_type == 'ifttt':
                        ...
                        #
                        # logging.info('---------------------------------------')
                        # logging.info('plotting diagrams...')
                        # logging.info('---------------------------------------')
                        #
                        # train_bleu_all_trans = np.transpose(train_bleu_all)
                        # train_acc_all_trans = np.transpose(train_acc_all)
                        # train_loss_all_trans = np.transpose(train_loss_all)
                        # val_bleu_all_trans = np.transpose(val_bleu_all)
                        # val_acc_all_trans = np.transpose(val_acc_all)
                        # val_loss_all_trans = np.transpose(val_loss_all)
                        #
                        # plt.figure(1)
                        # plt.plot(train_bleu_all_trans[0], train_bleu_all_trans[1], 'r', label="Training Bleu")
                        # plt.plot(val_bleu_all_trans[0], val_bleu_all_trans[1], 'b', label="Validation Bleu")
                        ...
                        # plt.plot(val_acc_all_trans[0], val_acc_all_trans[1], 'b', label="Validation Accuracy")
                        ...
                        # plt.plot(val_loss_all_trans[0], val_loss_all_trans[1], 'b', label="Validation Loss")
            ...
            # # train_bleu_cur_epoch = [epoch, bleu]
            # # train_acc_cur_epoch = [epoch, accuracy]
